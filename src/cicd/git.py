...
from cicd._internal import Utils
...
class GitDescription:
    ...
    @classmethod
    def call(cls: type[Self], repo_dir: Path) -> Self:
...
class GitConfig:
    ...
    def call(cls: type[Self]) -> Self:
        return cls(
            ...
            Utils.parse_boolean(_call(["git", "config", "commit.gpgsign"])),
            Utils.parse_boolean(_call(["git", "config", "pull.rebase"])),
        )
...
class GitClone:
    ...
    @classmethod
    def call(cls: type[Self], repo_url: str, repo_path: Path) -> Self:
